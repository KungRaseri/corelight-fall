...
export const choice = pgTable('choice', {
    ...
    encounterId: integer('encounter_id').references(() => encounter.id, {onDelete: 'cascade'}).notNull(),
    text: text('text').notNull(), // The choice text shown to the player
    nextEncounterId: integer('next_encounter_id').references(() => encounter.id, { onDelete: 'cascade' }), // null if end
