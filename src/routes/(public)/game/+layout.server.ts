	...
	if (progress) {
		...
		currentQuest = (
			await db
				.select()
				.from(quest)
				.where(eq(quest.id, progress.questId ?? -1))
		)[0];
		currentEncounter = (
			await db
				.select()
				.from(encounter)
				.where(eq(encounter.id, progress.encounterId ?? -1))
		)[0];
		availableChoices = await db
			.select()
			.from(choice)
			.where(eq(choice.encounterId, progress.encounterId ?? -1));
		...
		currentQuest = (
			await db
				.select()
				.from(quest)
				.where(eq(quest.storylineId, currentStoryline.id))
				.orderBy(quest.order)
				.limit(1)
		)[0];
		currentEncounter = (
			await db
				.select()
				.from(encounter)
				.where(eq(encounter.questId, currentQuest.id))
		...
		availableChoices = await db
			.select()
			.from(choice)
			.where(eq(choice.encounterId, currentEncounter.id));
	}
