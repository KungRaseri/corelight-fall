...
export const load: PageServerLoad = async ({ locals }) => {
    ...
    const inventory = await db.select({
        ...
        .leftJoin(characterEquipment, eq(characterItem.characterId, locals.character.id) && eq(characterItem.itemId, item.id))
        .where(eq(characterItem.characterId, locals.character.id));
